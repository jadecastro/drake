...
class AgentTrajectoryValues final {
 public:
  ...
  Eigen::Vector3d pose3() const {
    const Vector4<double> quat{rotation_.w(), rotation_.x(), rotation_.y(),
                               rotation_.z()};
    const double w_z = math::QuaternionToSpaceXYZ(quat).z();
 ...
 private:
  ...
  AgentTrajectoryValues(const Eigen::VectorXd vector)
...
enum class InterpolationType { ZeroOrderHold, FirstOrderHold, Cubic, Pchip };
...
class AgentTrajectory final {
 public:
  ...
  static AgentTrajectory Make(const InterpolationType& interp_type,
    ...
    switch (interp_type) {
      case InterpolationType::ZeroOrderHold:
        return AgentTrajectory(
            PiecewisePolynomial<double>::ZeroOrderHold(times, knots_vector));
      case InterpolationType::FirstOrderHold:
        return AgentTrajectory(
