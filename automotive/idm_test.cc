...
add_oval_road(const unsigned &num_lanes) {
  const unsigned lane_width = 5;
  ...
  auto c0 = builder.Connect("0", num_lanes, 0, 2., 2., start, 200, LayFlat);
  auto c1 = builder.Connect("1", num_lanes, 0, 2, 2, c0->end(),
                            counter_clockwise_arc, LayFlat);
  auto c2 = builder.Connect("2", num_lanes, 0, 2, 2, c1->end(), 200, LayFlat);
  builder.Connect("3", num_lanes, 0, 2, 2, c2->end(),
                  counter_clockwise_arc, LayFlat);
  //builder.Connect("4",num_lanes,0,2,2,c3->end(),50,c0->start());

  std::unique_ptr<const drake::maliput::api::RoadGeometry> oval = builder.Build(
      drake::maliput::api::RoadGeometryId{"oval"});
  return oval;
}

std::unique_ptr<const drake::maliput::api::RoadGeometry> add_dragway(
    const unsigned & length, const unsigned & num_lanes){
  const double kMaximumHeight = 5.;  // meters
  const unsigned lane_width = 5;
  const unsigned shoulder_width = 2;
...
void add_idm_car(std::string car_name, const drake::maliput::api::Lane * lane,
                 const double & lane_pos, const double & speed,
...
int main(){
  ...
  const unsigned idm_1_lane = 0;
  const unsigned idm_2_lane = 1;
  // const unsigned idm_3_lane = 2;
