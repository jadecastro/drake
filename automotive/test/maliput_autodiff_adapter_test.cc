...
GTEST_TEST(MaliputAutodiffUtils, TestDoubleMonolane) {
  ...
  const Lane* lane = GetLaneFromId(*rg, "l:pre0");
...
GTEST_TEST(MaliputAutodiffUtils, TestAutoDiffMonolane) {
  // N.B. In this road, `post0` branches into `pre0` and `onramp1`.
  std::unique_ptr<MonolaneOnrampMerge> merge_road(new MonolaneOnrampMerge);
  std::unique_ptr<const maliput::api::RoadGeometry> rg =
      merge_road->BuildOnramp();
  ...
  for (int i{0}; i < 1; ++i) {
    const Lane* lane = GetLaneFromId(*rg, "l:post0"); // rg->junction(i)->segment(0)->lane(0);
