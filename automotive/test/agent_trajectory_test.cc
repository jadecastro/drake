...
GTEST_TEST(AgentTrajectoryTest, MismatchedSizes) {
  ...
  EXPECT_THROW(
      AgentTrajectory::Make(InterpolationType::FirstOrderHold, times, values),
      std::exception);
}
...
GTEST_TEST(AgentTrajectoryTest, InterpolationType) {
  ...
  std::vector<AgentTrajectoryValues> values{dummy_value, dummy_value,
                                            dummy_value};
  for (const auto& type :
       {Type::ZeroOrderHold, Type::FirstOrderHold, Type::Cubic, Type::Pchip}) {
...
GTEST_TEST(AgentTrajectoryTest, AgentTrajectory) {
  ...
  for (const double time : times) {
    translations.push_back({1. + time * 0., 2., 3.});
    rotations.push_back({4., 5., 6., 7.});
    rotations.back().normalize();
    w_vector.push_back({8., 9., 10.});
    v_vector.push_back({11., 12., 13.});
