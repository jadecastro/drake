...
template <typename T>
void SetAcceleration(const PathFollowingCar<T>& car_dut,
...
GTEST_TEST(PathFollowingCarTest, StationaryTest) {
  const std::vector<Point2d> empty_waypoints{};
  const Curve2d empty_curve{empty_waypoints};
  EXPECT_THROW((PathFollowingCar<double>{empty_curve}), std::exception);
}

template <typename T>
class PathFollowingCarTest : public ::testing::Test {};

typedef ::testing::Types<double, AutoDiffXd> Implementations;
TYPED_TEST_CASE(PathFollowingCarTest, Implementations);
...
// its own unit test.
TYPED_TEST(PathFollowingCarTest, ConstantSpeedTest) {
  using T = TypeParam;
  ...
  for (const auto& it : cases) {
    ...
    const PathFollowingCar<T> car_dut{curve};
    ...
    auto car_state = dynamic_cast<PathFollowingCarState<T>*>(
...
TYPED_TEST(PathFollowingCarTest, AccelerationInputTest) {
  using std::abs;
  using T = TypeParam;
  ...
  const std::vector<Point2d> waypoints{{10., 20.},  // BR
                                       {10., 30.}};
  const Curve2d curve{waypoints};
  const PathFollowingCar<T> dut{curve};  // The device under test.

  auto context = dut.CreateDefaultContext();
  auto derivatives = dut.AllocateTimeDerivatives();
  auto car_derivatives = dynamic_cast<const PathFollowingCarState<T>*>(
      &derivatives->get_mutable_vector());
  ASSERT_NE(nullptr, car_derivatives);
  const PathFollowingCarParams<T> default_params;
...
// bounds.
TYPED_TEST(PathFollowingCarTest, SaturatingSpeedTest) {
  using T = TypeParam;
...
GTEST_TEST(PathFollowingCarTest, ToAutoDiff) {
  const std::vector<Point2d> waypoints{{10., 20.},  // BR
                                       {10., 30.}};
  const Curve2d curve{waypoints};
  const PathFollowingCar<double> dut{curve};
...
GTEST_TEST(PathFollowingCarTest, ToSymbolic) {
  const std::vector<Point2d> waypoints{{10., 20.},  // BR
                                       {10., 30.}};
  const Curve2d curve{waypoints};
  const PathFollowingCar<double> dut{curve};
