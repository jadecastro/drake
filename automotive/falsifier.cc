...
void Falsifier::Run() {
  ...
  trajectory_.inputs = prog.GetInputSamples();
  trajectory_.states = prog.GetStateSamples();
  trajectory_.times = prog.GetSampleTimes();
  Eigen::VectorXd ego_x = trajectory_.states.row(ego_indices().at(kX));
  Eigen::VectorXd ego_y = trajectory_.states.row(ego_indices().at(kY));
  ...
  for (int i{0}; i < num_ado_cars; ++i) {
    Eigen::VectorXd ado_x = trajectory_.states.row(ado_indices(i).at(kX));
    Eigen::VectorXd ado_y = trajectory_.states.row(ado_indices(i).at(kY));
