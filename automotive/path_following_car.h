...
#include "drake/automotive/gen/path_following_car_params.h"
...
/// PathFollowingCar models a car that follows a pre-established trajectory.
/// Note that PathFollowingCar can move forward (up to a given "soft" speed
/// limit) but cannot travel in reverse.
///
/// parameters:
/// * uses systems::Parameters wrapping a PathFollowingCarParams
///
/// state vector:
/// * A PathFollowingCarState, consisting of a position and speed along the given
...
///   specified in the PathFollowingCarState.
///
/// output port 0:
...
template <typename T>
class PathFollowingCar final : public systems::LeafSystem<T> {
 public:
  typedef typename Curve2<T>::Point2T Point2;

  DRAKE_NO_COPY_NO_MOVE_NO_ASSIGN(PathFollowingCar)

  /// Constructs a PathFollowingCar system that traces a given two-dimensional
  /// @p curve.  Throws an error if the curve is empty (has a zero @p
  /// path_length).
  explicit PathFollowingCar(const Curve2<double>& curve)
      : systems::LeafSystem<T>(
            systems::SystemTypeTag<automotive::PathFollowingCar>{}),
    ...
    this->DeclareVectorOutputPort(&PathFollowingCar::CalcStateOutput);
    this->DeclareVectorOutputPort(&PathFollowingCar::CalcPoseOutput);
    this->DeclareVectorOutputPort(&PathFollowingCar::CalcVelocityOutput);
    this->DeclareContinuousState(PathFollowingCarState<T>());
    this->DeclareNumericParameter(PathFollowingCarParams<T>());
  }

  /// Scalar-converting copy constructor.  See @ref system_scalar_conversion.
  template <typename U>
  explicit PathFollowingCar(const PathFollowingCar<U>& other)
      : PathFollowingCar<T>(Curve2<double>(other.curve_.waypoints())) {}
 ...
 private:
  ...
  void CalcStateOutput(const systems::Context<T>& context,
    ...
    const PathFollowingCarState<T>& state = GetState(context);
  ...
  void DoCalcTimeDerivatives(
    ...
    const PathFollowingCarParams<T>& params =
        this->template GetNumericParameter<PathFollowingCarParams>(context, 0);

    // Obtain the state.
    const PathFollowingCarState<T>* const state =
        dynamic_cast<const PathFollowingCarState<T>*>(
    ...
    PathFollowingCarState<T>* const rates =
        dynamic_cast<PathFollowingCarState<T>*>(&vector_derivatives);
    DRAKE_ASSERT(rates);
  ...
  template <typename> friend class PathFollowingCar;

  void ImplCalcOutput(const PositionHeading& raw_pose,
                      const PathFollowingCarState<T>& state,
  ...
  void ImplCalcTimeDerivatives(const PathFollowingCarParams<T>& params,
                               const PathFollowingCarState<T>& state,
                               const systems::BasicVector<T>& input,
                               PathFollowingCarState<T>* rates) const {
    using std::max;
  ...
  const PathFollowingCarState<T>& GetState(
      const systems::Context<T>& context) const {
    auto state = dynamic_cast<const PathFollowingCarState<T>*>(
        &context.get_continuous_state_vector());
  ...
  const PositionHeading CalcRawPose(
      const PathFollowingCarState<T>& state) const {
    using std::atan2;
...
template <>
struct Traits<automotive::PathFollowingCar> : public NonSymbolicTraits {};
}  // namespace scalar_conversion
}  // namespace systems
