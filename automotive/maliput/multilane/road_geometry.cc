...
// Updates the `road_position`, `distance` and `nearest_position` (when it's
// not nullptr) if one of the following conditions are met after computing
// ToLanePosition on `lane` for `geo_position`:
//
// - When the distance on the lane is just smaller than *`distance` and it is
// different from 0.
// - When the distance on the lane is 0, and the r coordinate of the
// LanePosition is within the range of the lane bounds of `lane`. Note that the
// the bounds are computed as: [min, max). The range is selected on purpose, so
// there is no duplicates on lane assignment (i.e. `geo_position` is just over
// the lane bound limit).
// - When the distance on the lane is 0, and the r coordinate is outside the
// range of lane bounds but the absolute r coordinate is smaller than the
// absolute r coordinate of LanePosition that comes in `road_position`.
//
