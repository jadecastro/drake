...
class DiagramTester : public System<T>,
 ...
 private:
  ...
  struct StateMachine {
    ...
    std::vector<std::unique_ptr<System<T>>> modal_subsystems;

  };

  // Constructs a Diagram from the StateMachine that a DiagramBuilder produces.
  // This constructor is private because only DiagramBuilder calls it.
  explicit DiagramTester(const StateMachine& state_machine) {
    Initialize(state_machine);
  }

  // Validates the given @p state_machine and sets up the Diagram accordingly.
  ...
  DiagramTester(const DiagramTester<T>& other) = delete;
  DiagramTester& operator=(const DiagramTester<T>& other) = delete;
  DiagramTester(DiagramTester<T>&& other) = delete;
  DiagramTester& operator=(DiagramTester<T>&& other) = delete;
  ...
  friend class DiagramBuilderTester<T>;
};
