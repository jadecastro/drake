...
TimeVaryingLinearQuadraticRegulator(
  ...
  TimeVaryingLqrResult lqr_result = TimeVaryingLinearQuadraticRegulator(
      linear_system->A(), linear_system->B(), Q, R, N);

  // Return the affine controller: u(t) = u0(t) - K(t)(x(t) - x0(t)).
  return std::make_unique<systems::TimeVaryingStateFeedbackController<double>>(
      lqr_result.K, x0, u0, sample_time);
}
