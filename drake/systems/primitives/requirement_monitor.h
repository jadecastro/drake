...
class RequirementsMonitor : public LeafSystem<double> {
 public:
  ...
  void DoCalcNextUpdateTime(const systems::Context<T>& context,
    ...
    // If a constraint violation is detected, set the event time to the next
    // sample time.  Otherwise, keep simulating.
  ...
  void DoCalcUnrestrictedUpdate(const systems::Context<T>& context,
    ...
    // For now, let's screen-dump the value of the continuous states, and then
    // halt the simulation.
    std::cerr << " state:" << std::endl;
    for (const auto value : cstate)
