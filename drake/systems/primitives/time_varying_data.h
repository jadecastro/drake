...
std::vector<double> vector_iota(int size, double step) {
  DRAKE_DEMAND(step > 0.);
...
// Return an Eigen vector of zeros of the given size.
...
// Eigen::MatrixXd stored in pp.
PiecewisePolynomial<double> MakeZeroedPiecewisePolynomial(
...
struct TimeVaryingData {
  ...
  /// element corresponds to time = i * time_period.  The matrix values are
  /// interpolated linearly in between time steps.
  ///
...
struct LinearTimeVaryingData : TimeVaryingData {
  ...
  /// Fully-parameterized constructor of PiecewisePolynomialTrajectories.
  // TODO(jadecastro) Generalize to Trajectories.
  LinearTimeVaryingData(const PiecewisePolynomialTrajectory& A,
      ...
      : TimeVaryingData(A, B,
                        PiecewisePolynomialTrajectory(
                            internal::MakeZeroedPiecewisePolynomial(
                                A.get_piecewise_polynomial())),
                        C, D,
                        PiecewisePolynomialTrajectory(
                            internal::MakeZeroedPiecewisePolynomial(
                                C.get_piecewise_polynomial()))) {}
};
