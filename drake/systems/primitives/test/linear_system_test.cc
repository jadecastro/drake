...
TEST_F(TestLinearizeFromAffine, ContinuousAtEquilibrium) {
  ...
  EXPECT_TRUE(CompareMatrices(Eigen::Vector3d::Zero(), linearization.f0, tol,
                              MatrixCompareType::absolute));
  // Verify that the affine term y0 is just the original output equation.
  EXPECT_TRUE(CompareMatrices(C_ * x0 + D_ * u0_ + y0_, linearization.y0, tol,
                              MatrixCompareType::absolute));
}
...
TEST_F(TestLinearizeFromAffine, ContinuousAtNonEquilibrium) {
  ...
  double tol = 1e-10;
  EXPECT_TRUE(CompareMatrices(A_, linearization.linear_system->A(), tol,
                              MatrixCompareType::absolute));
  EXPECT_TRUE(CompareMatrices(B_, linearization.linear_system->B(), tol,
                              MatrixCompareType::absolute));
  EXPECT_TRUE(CompareMatrices(C_, linearization.linear_system->C(), tol,
                              MatrixCompareType::absolute));
  EXPECT_TRUE(CompareMatrices(D_, linearization.linear_system->D(), tol,
                              MatrixCompareType::absolute));
  EXPECT_TRUE(CompareMatrices(A_ * x0_ + B_ * u0_ + f0_,
                              linearization.f0, tol,
                              MatrixCompareType::absolute));
  EXPECT_TRUE(CompareMatrices(C_ * x0_ + D_ * u0_ + y0_, linearization.y0, tol,
                              MatrixCompareType::absolute));
}
...
TEST_F(TestLinearizeFromAffine, DiscreteAtNonEquilibrium) {
  ...
  EXPECT_TRUE(CompareMatrices(C_ * x0_ + D_ * u0_ + y0_, linearization.y0, tol,
                              MatrixCompareType::absolute));
  EXPECT_EQ(time_period_, linearization.linear_system->time_period());
