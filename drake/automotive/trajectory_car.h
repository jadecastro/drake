...
class TrajectoryCar : public systems::LeafSystem<T> {
 public:
  ...
  explicit TrajectoryCar(const Curve2<T>& curve) : curve_(curve) {
 ...
 private:
  ...
  void ImplCalcTimeDerivatives(const TrajectoryCarParams<T>& params,
    ...
    // Note that accelerations of zero are passed through unaffected.
  ...
  TrajectoryCar<AutoDiffXd>* DoToAutoDiffXd() const override {
    ...
    return new TrajectoryCar<AutoDiffXd>(curve);
  }
