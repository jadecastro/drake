...
static void SetDefaultOnrampPoses(const GeoPosition& traffic_geo_position,
  ...
  const Eigen::Translation3d translation_ahead(
      traffic_geo_position.x() /* x */, traffic_geo_position.y() /* y */,
...
GTEST_TEST(PoseSelectorTest, OnrampMergeTest) {
  ...
  // Define the default poses.
  SetDefaultOnrampPoses(traffic_geo_post5, 10 /* traffic_vy */, &ego_pose,
  ...
  leading_odometry = PoseSelector::FindSingleClosestAheadAndInBranches(
      *road, ego_pose, ego_velocity, traffic_poses,
      1000. /* scan_ahead_distance */, &headway_distance);
  ...
  // Define the default poses.
  SetDefaultOnrampPoses(traffic_geo_pre0, -10 /* traffic_vy */, &ego_pose,
                        &ego_velocity, &traffic_poses);
