...
class PurePursuitControllerTest : public ::testing::Test {
 protected:
  void SetUp() override {
    ...
    // Store the LaneId.
    lane_id_.id = road_->junction(0)->segment(0)->lane(0)->id().id;

    // Initialize PurePursuitController with the dragway.
    dut_.reset(new PurePursuitController<double>(std::move(road_)));
  ...
  void SetDefaultInputs(const double y_position, const double yaw) {
    ...
    context_->FixInputPort(dut_->lane_input().get_index(),
                           systems::AbstractValue::Make(lane_id_));
...
TEST_F(PurePursuitControllerTest, Topology) {
  ...
  EXPECT_EQ(3 /* DrivingCommand input */, command_input_descriptor.size());
  ...
  EXPECT_EQ(3 /* DrivingCommand output */, command_output_descriptor.size());
}
...
TEST_F(PurePursuitControllerTest, Output) {
  ...
  EXPECT_EQ(0., command->throttle());  // pure pursuit does not alter throttle.
  EXPECT_EQ(0., command->brake());  // pure pursuit does not alter brake.

  // Set the offset to be to the left of the centerline with zero orientation.
  SetDefaultInputs(1., 0.);
  dut_->CalcOutput(*context_, output_.get());

  // Expect the car to steer toward the right (negative steering angle).
  EXPECT_GT(0., command->steering_angle());
  ...
  // Set the offset to be to the left of the centerline, oriented perpendicular
  // to the track.
