...
void IdmController<T>::DoCalcOutput(const systems::Context<T>& context,
  ...
  const systems::VectorBase<T>* ego_vector_input =
      this->EvalAbstractInput(context, this->get_ego_input().get_index());
  DRAKE_ASSERT(ego_vector_input != nullptr);
  const PoseVector<T>* const ego_pose =
      dynamic_cast<const PoseVector<T>*>(ego_vector_input);
  DRAKE_ASSERT(ego_pose != nullptr);

  // TODO(jadecastro): Make this PoseBundle and select a certain PoseVector.
  const systems::VectorBase<T>* agents_vector_input =
      this->EvalAbstractInput(context, this->get_agent_input().get_index());
  DRAKE_ASSERT(agents_vector_input != nullptr);
  const PoseVector<T>* const agents_pose =
      dynamic_cast<const PoseVector<T>*>(agents_vector_input);
  DRAKE_ASSERT(agents_pose != nullptr);

  systems::BasicVector<T>* const output_vector =
      output->GetMutableVectorData(0);
  DRAKE_ASSERT(output_vector != nullptr);
  const DrivingCommand<T>* const command =
      dynamic_cast<const DrivingCommand<T>*>(output_vector);
  DRAKE_ASSERT(command != nullptr);
  ...
  ImplDoCalcOutput(ego_pose, agents_pose, params, command);
}
...
void IdmController<T>::ImplDoCalcOutput(const PoseVector<T>& ego_pose,
  ...
  Eigen::Translation<double, 3> ego_position = ego_pose.get_translation();
  const T& x_ego = ego_position.translation().x();
  const T& v_ego = 0.;  // TODO(jadecastro): Require velocity from SimpleCar.
  Eigen::Translation<double, 3> agent_position = agents_pose.get_translation();
  const T& x_agent = agent_position.translation().x();
  const T& v_agent = 0.;  // TODO(jadecastro): Require velocity from SimpleCar.
  ...
  const T lead_range = x_agent - x_ego;
...
std::unique_ptr<systems::Parameters<T>> IdmController<T>::AllocateParameters()
    const {
  auto params = std::make_unique<IdmControllerParameters<T>>();
...
void IdmController<T>::SetDefaultParameters(
  ...
  auto idm_params = dynamic_cast<IdmControllerParameters<T>*>(
