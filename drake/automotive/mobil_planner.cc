...
template <typename T>
MobilPlanner<T>::MobilPlanner(const T& v_ref) : v_ref_(v_ref) {
...
template <typename T>
const systems::InputPortDescriptor<T>& MobilPlanner<T>::get_ego_port() const {
  return systems::System<T>::get_input_port(0);
}

template <typename T>
const systems::InputPortDescriptor<T>& MobilPlanner<T>::get_agent_port() const {
