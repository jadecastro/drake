...
using Domain = std::pair<Bound, Bound>;

/// Defines an autonomous cubic polynomial system:
///   ẋ = 100x³ - 25x
template <typename T>
...
Expression MakeVolumetricCost(const Polynomial& p, Domain b) {
...
/// Solves the backward reachable set over T seconds on the above 1D example,
/// given a terminal set.  Specifically, we solve the dual of the polynomial
/// relaxation to an infinite-dimensional LP over the space of occupation
/// measures capturing the reachable set.  The dual relaxation solves, over the
/// space of nonegative functions, one that tightly overapproximates the
/// reachable set.
///
/// For T = 1 and given the target set ‖x(T)‖ ≤ 0.1, the cubic polynomial system
/// ẋ = 100x³ - 25x is known to have a backward reachable set ‖x(0)‖ ≤ 0.5.
void ComputeBackwardReachableSet() {
  ...
  //      w ≥ 0 on X
  //
  // The result is the T-horizon overapproximated backward reachable set,
  //    B = X₀ = {x ∈ X | w ≥ 1}.
  ...
  // w is an overapproximation to B over x ∈ X.
  ...
  // w represents a *tight* overapproximation.
  ...
  cout << "    positive semidefinite constraints: "
  ...
  cout << " Optimal cost: " << prog.GetOptimalCost() << endl;

  Environment env;
  for (const auto& it : w.decision_variables()) {
    env.insert(it, prog.GetSolution(it));
  }
  const Polynomial w_sol{w.EvaluatePartial(env)};
  cout << " w solution: " << w_sol << endl;

  // Serialize the result and generate Python plots.
  // To plot, execute `bazel run //common/proto:call_python_client_cli`
  const int N{1000};
  Eigen::VectorXd x_val(N), w_val(N), ground(N);
  for (int i = 0; i < N; i++) {
    x_val[i] = x_bound * (2. * i / N - 1.);
    w_val[i] = w_sol.Evaluate({{x, x_val[i]}});
    ground[i] = (gx0.Evaluate({{x, x_val[i]}}) >= 0.);
  }
  CallPython("figure", 1);
  CallPython("clf");
  CallPython("plot", x_val, w_val);
  CallPython("setvars", "x_val", x_val, "w_val", w_val);
  CallPython("plot", x_val, ground);
  CallPython("setvars", "x_val", x_val, "ground", ground);
  CallPython("plt.show", true);
}
